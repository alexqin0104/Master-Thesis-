...
# ============================================================
# 1. Generate one gamma point process observation (point-by-point)
# ============================================================
# Consistent with SCED gamma extension: calls ARPobservation's
# event_counting(r_behavior_stream(...)) with stream_length = 1,
# generating one event count per unit window.
# Structurally parallel to gen_one_pois(prev, mu, phi) in the binomial thinning
# simulation, but gamma renewal process has no AR mechanism so prev_value is not used.
gen_one_gamma <- function(mu_i, shape) {
  if (is.na(mu_i) || mu_i <= 0) mu_i <- 0.5
...
# ============================================================
generate_case_data <- function(alg, mu_i, shape, absolute_effect,
                               min_start, max_bl = 40, treatment_length = 15) {
  
  # Generate initial baseline
  data <- numeric(min_start)
  for (t in 1:min_start) {
    data[t] <- gen_one_gamma(mu_i, shape)
  }
  ...
  if (!is.character(alg) || alg != "fixed") {
    repeat {
      ...
      bl_len <- bl_len + 1
      data[bl_len] <- gen_one_gamma(mu_i, shape)
    }
  } else {
  ...
  for (t in 1:treatment_length) {
    new_pt <- gen_one_gamma(mu_i, shape) + absolute_effect
...
# ============================================================
generate_fixed_case_data <- function(case_id, mu_i, shape, absolute_effect,
  ...
  for (t in 1:treatment_length) {
    tx_data[t] <- gen_one_gamma(mu_i, shape) + absolute_effect
  }
...
# ============================================================
sim_one_replicate_mbd <- function(alg, mu, shape, absolute_effect,
                                  n_cases = 4, min_bl = 5, min_stagger = 3,
                                  max_bl = 40, treatment_length = 15) {
  
  # Case-level random intercepts
  # Within-case stationary variance under gamma renewal process (Cox 1962; Swan 2020):
  #   var_i = (mu / shape) + (shape^2 - 1) / (6 * shape^2)
  # tau^2 set equal to var_i so that ICC = 0.5
  var_i <- (mu / shape) + (shape^2 - 1) / (6 * shape^2)
  sigma_r0 <- sqrt(var_i)
  ...
  if (is.character(alg) && alg == "fixed") {
    case_results <- lapply(1:n_cases, function(i) {
      generate_fixed_case_data(i, mu_i_vec[i], shape, absolute_effect,
  ...
  for (case_id in 1:n_cases) {
    ...
    cr <- generate_case_data(alg, mu_i_vec[case_id], shape, absolute_effect,
...
# ============================================================
sim_driver <- function(iterations, n, treatment_length, mu, shape, effect_size) {
  ...
  res_list <- map_dfr(names(alg_vec), function(alg_name) {
    ...
    reps <- map_dfr(1:iterations, function(rep_i) {
      ...
      case_results <- sim_one_replicate_mbd(
        alg = alg_func, mu = mu, shape = shape,
