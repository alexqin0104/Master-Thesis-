...
#These "algorithms" are used in order to create the fixed baselines, which serve
#as references for the other results
fixed5 <- function(outcomes, increase = TRUE){
  return(length(outcomes) >= 5)
}
fixed49 <- function(outcomes, increase = TRUE){
  return(length(outcomes) >= 49)
}
