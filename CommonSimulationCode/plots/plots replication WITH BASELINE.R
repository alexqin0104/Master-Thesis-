#this script is mainly recycled from the version without baseline.
rm(list = ls())
...
binomial_thinning_adaptive <- env1[[ls(env1)]]
gamma_point_process_adaptive <- env2[[ls(env2)]]
normal_errors_adaptive <- env3[[ls(env3)]]
...
binomial_thinning_fixed <- env1[[ls(env1)]]
gamma_point_process_fixed <- env2[[ls(env2)]]
normal_errors_fixed <- env3[[ls(env3)]]

#remerging, since we got the split data only
binomial_thinning_results = rbind(binomial_thinning_adaptive, binomial_thinning_fixed)
gamma_point_process_results = rbind(gamma_point_process_adaptive, gamma_point_process_fixed)
normal_errors_results = rbind(normal_errors_adaptive, normal_errors_fixed)
